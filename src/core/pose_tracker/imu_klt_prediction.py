...
def integrate_gyro_delta_rotation(
    ...
    gyro_batch_float64 = np.asarray(gyro_batch, dtype=np.float64)
    imu_ts_batch = np.asarray(imu_ts_batch, dtype=np.float64)
    if gyro_batch_float64.shape[0] != imu_ts_batch.shape[0]:
        msg = "gyro_batch and imu_ts_batch must have the same number of rows"
        raise ValueError(msg)
    if gyro_batch_float64.shape[0] == 0:
    ...
    for gyro, timestamp in zip(gyro_batch_float64, imu_ts_batch, strict=True):
...
def predict_points_by_gyro(
    ...
    points_float64 = np.asarray(points, dtype=np.float64)
    if points_float64.ndim != POINTS_NDIM or points_float64.shape[1] != POINT2_WIDTH:
        msg = "points must have shape (N, 2)"
        raise ValueError(msg)
    if points_float64.shape[0] == 0:
    ...
    hom_points = np.column_stack((points_float64, np.ones(points_float64.shape[0], dtype=np.float64)))
    rays_prev = (np.linalg.inv(k_matrix) @ hom_points.T).T
    rays_next = camera_next_from_prev.apply(rays_prev)

    prediction = np.full(points_float64.shape, np.nan, dtype=np.float64)
