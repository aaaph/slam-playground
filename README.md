...
evo_ape tum <ground_truth.tum> <estimate.tum> -a --plot_mode xy --save_plot results/<selector>_traj.png
```

The first command writes `ape.json` / `ape.txt` / `ape.zip` next to the run under `pipeline/out/<run-id>/evo/` — copy `stats.rmse` and `stats.max` from there into the table below. The second renders the top-down trajectory plot (estimate vs. ground truth) referenced in the `Trajectory` column.

| Selector | Sequence | Difficulty | RMSE (m) | Max (m) | Trajectory |
| --- | --- | --- | --- | --- | --- |
| `euroc_mh_01` … `euroc_mh_05` | Machine Hall | easy → difficult | _TBD_ | _TBD_ | _TBD_ |
| `euroc_v101` | Vicon Room 1_01 | easy | 0.057589 | _TBD_ | <a href="./results/euroc_v101_traj.png"><img src="./results/euroc_v101_traj.png" width="160"></a> |
| `euroc_v102` | Vicon Room 1_02 | medium | _TBD_ | _TBD_ | _TBD_ |
| `euroc_v103` | Vicon Room 1_03 | difficult | _TBD_ | _TBD_ | _TBD_ |
| `euroc_v201` … `euroc_v203` | Vicon Room 2 | easy → difficult | _TBD_ | _TBD_ | _TBD_ |
