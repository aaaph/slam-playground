...
datascience_image = modal.Image.debian_slim().uv_pip_install("pandas==2.2.0", "numpy")
...
```python
datascience_image = modal.Image.debian_slim(python_version="3.13").pip_install("pandas==2.2.0", "numpy")
```
...
def download_models() -> None:
    ...
    pipe = diffusers.StableDiffusionPipeline.from_pretrained(model_name, use_auth_token=os.environ["HF_TOKEN"])
...
```python
image = (
    modal.Image.debian_slim().apt_install("git").pip_install("slack-sdk", force_build=True).run_commands("echo hi")
)
```
