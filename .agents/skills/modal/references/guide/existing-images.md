...
```
REGISTRY_USERNAME_EXAMPLE=my-dockerhub-username
REGISTRY_PASSWORD_EXAMPLE=my-password-example
```
...
image = modal.Image.debian_slim().pip_install("foo").entrypoint(["/usr/bin/my_entrypoint.sh"])
```
