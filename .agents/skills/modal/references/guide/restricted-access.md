...
```python
@app.function(restrict_modal_access=True, block_network=True, single_use_containers=True)
...
image = modal.Image.debian_slim().add_local_file("restricted_executor.py", "/root/restricted_executor.py")


@restricted_app.function(restrict_modal_access=True, block_network=True, single_use_containers=True)
def isolated_function(input_data):
    return process(input_data)
```
