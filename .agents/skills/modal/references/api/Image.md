...
# exclude large files
modal.Image.debian_slim().add_local_python_source("mymodule", ignore=lambda p: p.stat().st_size > 1e9)
```
...
```python
micromamba_install(
    self, *packages, spec_file=None, channels=[], force_build=False, env=None, secrets=None, gpu=None
)
```
...
```python
apt_install(self, *packages, force_build=False, env=None, secrets=None, gpu=None)
```
...
image = (
    modal.Image.debian_slim().pip_install("torch").run_function(my_build_function, secrets=[...], volumes={...})
)
```
...
```python
image = modal.Image.debian_slim().env({"HF_HUB_ENABLE_HF_TRANSFER": "1"})
```
...
```python
image = modal.Image.debian_slim().cmd(["python", "app.py"])
```
...
image = modal.Image.debian_slim().apt_install("git").pipe(workspace_setup, "https://github.com/example/repo.git")
```
