...
stt_image = (
    ...
    .uv_pip_install("moshi==0.2.9", "fastapi==0.116.1", "huggingface-hub==0.33.5", "julius==0.2.7")
...
class STT:
    ...
    def enter(self):
        ...
        self.audio_silence_prefix_seconds = checkpoint_info.stt_config.get("audio_silence_prefix_seconds", 1.0)
        self.audio_delay_seconds = checkpoint_info.stt_config.get("audio_delay_seconds", 5.0)
        self.padding_token_id = checkpoint_info.raw_config.get("text_padding_token_id", 3)

        # warmup gpus
        for _ in range(4):
            codes = self.mimi.encode(torch.zeros(self.BATCH_SIZE, 1, self.frame_size).to(self.device))
    ...
    async def transcribe(self, pcm, all_pcm_data):
        ...
        while all_pcm_data.shape[-1] >= self.frame_size:
            ...
            with torch.no_grad():
                ...
                chunk = chunk.expand(self.BATCH_SIZE, -1, -1)  # (batch_size, 1, frame_size)
                ...
                for c in range(codes.shape[-1]):
                    text_tokens, vad_heads = self.lm_gen.step_with_extra_heads(codes[:, :, c : c + 1])
        ...
        async def transcribe_websocket(ws: WebSocket):
            ...
            async def send_loop():
                ...
                while True:
                    ...
                    msg = b"\x01" + bytes(data, encoding="utf8")  # prepend "\x01" as a tag to indicate text
...
web_image = (
    ...
    .add_local_dir(Path(__file__).parent / "streaming-kyutai-stt-frontend", "/root/frontend")
)
```
...
def ui():
    ...
    fast_app, rt = fh.fast_app(
        hdrs=[
            ...
            fh.Script(src="https://cdn.jsdelivr.net/npm/opus-recorder@latest/dist/recorder.min.js"),
            fh.Script(src="https://cdn.jsdelivr.net/npm/opus-recorder@latest/dist/encoderWorker.min.js"),
            fh.Script(src="https://cdn.jsdelivr.net/npm/ogg-opus-decoder/dist/ogg-opus-decoder.min.js"),
            # styling
            fh.Link(
