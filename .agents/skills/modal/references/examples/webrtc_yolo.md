...
![WebRTC on Modal](https://modal-cdn.com/cdnbot/webrtcdv9r193o_8efc6c14.webp) <small>A clean architecture for WebRTC on Modal</small>
...
The signaling request returns as soon as the GPU peer publishes an SDP *answer*.
...
class ObjDet:
    ...
    async def run_pipeline(self, d: modal.Dict):
        ...
        ice_servers = [IceServer(**ice_server) for ice_server in await d.get.aio("ice_servers")]
        ...
        async def on_client_connected(transport, client):
            print(f"Video Processor connection {webrtc_connection.pc_id}: client connected")
...
class WebcamObjDet:
    ...
    def web(self):
        ...
        async def ice_servers(mode: str = "stun"):
            try:
                return {"ice_servers": await resolve_ice_servers(use_turn=(mode == "turn"))}
...
def ice_servers_for_mode(use_turn: bool) -> list[dict]:
    ...
    if not username or not credential:
        raise RuntimeError("TURN mode needs Modal Secret 'turn-credentials' (TURN_USERNAME, TURN_CREDENTIAL)")
