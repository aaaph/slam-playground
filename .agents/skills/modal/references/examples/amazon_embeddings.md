...
```python
@app.function(image=modal.Image.debian_slim().uv_pip_install("datasets==3.5.1"), timeout=2 * HOURS)
...
MODEL_DIR = "/model"
MODEL_CACHE_VOLUME = modal.Volume.from_name("embeddings-example-model-cache", create_if_missing=True)
...
def generate_batches_of_chunks(dataset, chunk_size: int = CHUNK_SIZE, batch_size: int = BATCH_SIZE):
