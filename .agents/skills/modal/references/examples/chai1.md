...
```python
chai_model_volume = modal.Volume.from_name(  # create distributed filesystem for model weights
    "chai1-models",
...
def chai1_inference(fasta_content: str, inference_config: dict, run_id: str) -> list[(bytes, str)]:
    from pathlib import Path
    ...
    print(f"🧬 done, results written to /{output_dir.relative_to('/preds')} on remote volume")
