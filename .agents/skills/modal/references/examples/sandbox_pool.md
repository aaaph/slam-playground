...
def add_sandbox_to_queue() -> None:
    ...
    sb = modal.Sandbox.create(
        *sandbox_cmd,
        ...
        readiness_probe=modal.Probe.with_exec("curl", "-sf", f"http://localhost:{SANDBOX_SERVER_PORT}/"),
    )
...
def claim_sandbox(check_health: bool = True) -> str:
    ...
    while True:
        print("Adding a new Sandbox to the pool to backfill (and ensure we have at least one)...")
...
def maintain_pool():
    ...
    if diff > 0:
        ...
        terminate_sandboxes.spawn([sr.id for sr in pool_queue.get_many(n_values=-diff, timeout=0)])
...
def demo():
    ...
    deployed_web_function = modal.Function.from_name(APP_NAME, "claim_sandbox_web_function")
