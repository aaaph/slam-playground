...
def load_or_cache_dataset(config: "TrainingConfig", paths: dict, tokenizer):
    ...
    if dataset_cache_path.exists():
        ...
        dataset = dataset.train_test_split(test_size=1.0 - TRAIN_SPLIT_RATIO, seed=config.seed)
...
def get_structured_paths(config: TrainingConfig):
    ...
    dataset_cache_path = pathlib.Path("/dataset_cache") / "datasets" / config.dataset_name.replace("/", "--")
