...
class Vllm:
    ...
    def process(self, filings: list[Filing]) -> list[Filing]:
        ...
        out_token_count = sum(len(response.outputs[0].token_ids) for response in responses)
...
```python
sec_edgar_feed = modal.Volume.from_name("example-sec-edgar-daily", create_if_missing=True, version=2)
data_root = Path("/data")
```
...
```python
@app.function(volumes={data_root: sec_edgar_feed}, timeout=10 * MINUTES, scaledown_window=5)
...
```python
scraper_image = modal.Image.debian_slim(python_version="3.13").uv_pip_install("requests==2.32.5")
```
