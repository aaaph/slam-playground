...
def create_endpoint_if_missing() -> None:
    ...
    endpoints = json.loads(subprocess.check_output([*command, "list", "--json"], text=True))
...
def test_session(
    ...
    with urllib.request.urlopen(request, timeout=SESSION_START_TIMEOUT + 60) as response:
