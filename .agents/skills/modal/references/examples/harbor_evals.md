...
```python
def main() -> None:
    parser = argparse.ArgumentParser(description="Run parallel Harbor oracle evals on Modal Sandboxes.")
    ...
    if not has_toml and not has_env:
        print(
            "Modal auth required: run `modal token new` or set MODAL_TOKEN_ID and MODAL_TOKEN_SECRET.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(
            f"Running {args.n_parallel} trials on {args.n_parallel} concurrent Modal Sandboxes...",
            flush=True,
        )
        subprocess.run(
            [
