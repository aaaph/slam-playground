...
```python
SGLANG_IMAGE = (
    "lmsysorg/sglang:dev-inkling-dspark@sha256:fbea1a4e25b26660dbc2384a27ead8817e9b7670f257b5c3143e0450d14524d7"
)
...
compile_cache_vol = modal.Volume.from_name("inkling-compile-cache", create_if_missing=True)
...
def wait_for_server_ready(proc: subprocess.Popen):
    ...
    while True:
        ...
        if proc.poll() is not None:
            raise RuntimeError(f"SGLang exited with code {proc.returncode} before becoming healthy")
        if is_server_up(url):
