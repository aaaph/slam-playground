...
```python
image = image.env({"ACESTEP_PROJECT_ROOT": "/opt/ace-step", "HF_HUB_ENABLE_HF_TRANSFER": "1"})
```
...
def main(
    ...
    clip = music_generator.run.remote(prompt, lyrics, duration=duration, format=format, manual_seeds=manual_seeds)
...
def ui():
    ...
    async def generate_music(prompt: str, lyrics: str, duration: float = 30.0, format: str = "mp3"):
        audio_bytes = await generate.aio(prompt, lyrics, duration=duration, format=format)
    ...
    with gr.Blocks(theme="soft") as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                duration = gr.Number(label="Duration (seconds)", value=10.0, minimum=1.0, maximum=300.0)
