...
sglang_image = (
    ...
    .uv_pip_install(f"git+https://github.com/sgl-project/sglang.git@{GIT_SHA}#subdirectory=python")
)
```
...
def warmup():
    ...
    for _ in range(3):
        requests.post(f"http://127.0.0.1:{PORT}/v1/chat/completions", json=payload, timeout=10).raise_for_status()
```
